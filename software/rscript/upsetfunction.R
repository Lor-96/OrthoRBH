upsetplotfunction <- function(mgi_db, odb_db,  biomart_db, orthorbh, exalign, cds, cdsnotcommon ) {
  ...
  upsetdb <- function(mgi, odb, biomart, rbh){
    listInput2 <- list(RBH =rbh$RBH , MGI =mgi$MGI , ODB =odb$OrthoDB, Biomart=biomart$Biomart)
  ...
  upsetplotcdsnotcom <- function(mgi_db, odb, rbh, biomart, cdsnotcom){
    listInput1 <- list(CDS =cdsnotcom$CDS , MGI =mgi$MGI , ODB =odb$OrthoDB, RBH=rbh$RBH, Biomart=biomart$Biomart)
  ...
  upsetplotall <- function(mgi, odb, rbh, biomart, exalign_rbh, cds_rbh){
    listInput <- list(MGI =mgi$MGI , ODB =odb$OrthoDB, RBH_Protein= rbh$RBH, Biomart=biomart$Biomart,Exalign_RBH = exalign_rbh$Exalign_RBH, CDS_RBH=cds_rbh$CDS_RBH)

    tiff("part2_MGI_RBH_ODB_Biomart_Exalign_CDS_genenames.tiff", res=300, width = 1600 , height = 1500)
    print(upset(fromList(listInput), nsets = 6, order.by ="freq", number.angles = 40, point.size = 2.5, line.size = 1, 
          mainbar.y.label = "Orthologs Intersections", sets.x.label = "Orthologs Per Database", text.scale = c(0.5,0.5, 0.5, 0.5, 0.5, 0.5)))
